...
import '../bloc/who_us_bloc.dart';
import '../repo/who_us_repo.dart';
import '../../../main_widgets/question_card.dart';

class WhoUsPage extends StatelessWidget {
  const WhoUsPage({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(title: getTranslated("who_us")),
      body: BlocProvider(
        create: (context) => WhoUsBloc(
            repo: sl<WhoUsRepo>(), internetConnection: sl<InternetConnection>())
          ..add(Click()),
        child: Column(
          ...
          children: [
            ...
            Expanded(
              child: BlocBuilder<WhoUsBloc, AppState>(
                builder: (context, state) {
                  ...
                  if (state is Error || state is Empty) {
                    return RefreshIndicator(
                      ...
                      onRefresh: () async {
                        context.read<WhoUsBloc>().add(Click());
                      },
                      child: Column(
                        children: [
                          Expanded(
